...
def detect(net, meta, im, thresh=.5, hier_thresh=.5, nms=.45):
    ...
    for j in range(num):
        # sort 
        probarray = dets[j].prob
        max_index = probarray.index(max(probarray))
        classname = meta.names[max_index]
        prob = dets[j].prob[max_index]
        b = dets[max_index].bbox
        box = (b.x, b.y, b.w, b.h)
        res.append((classname,prob,box))
    free_image(im)
...
if __name__ == "__main__":
    ...
    import time
    path = input("input your video road : ")
    cap = cv2.VideoCapture(path)
    while True:
        ret,frame = cap.read()
        detect(net,meta,frame)
        ...
        for r in res:
            ...
            cv2.rectangle(frame,(x1,y1),(x2,y2),'r',3)
            cv2.putText(frame,classname,(x1,y1),2,1,'r',2)
