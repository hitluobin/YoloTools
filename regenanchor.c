...
void smart_centers(matrix data, matrix centers) {
	...
	double *weights = (double*)calloc(data.rows, sizeof(double));
...
void random_centers(matrix data, matrix centers) { //k-means
	int i;
...
int main(int argc, char *argv[])
{
	...
	int k = find_int_arg(argc, argv, (char *)"-k", 2);
	int header = find_arg(argc, argv, (char *)"-h");
	int count = find_arg(argc, argv, (char *)"-c");
...
{
	int i;
	double *field = (double *)calloc(n, sizeof(double));
	for (i = 0; i < n; ++i) {
		field[i] = atof(l);
		l = strchr(l, ',') + 1;
	}
...
matrix make_matrix(int rows, int cols)
{
	...
	m.rows = rows;
	m.cols = cols;
	m.vals = (double **)calloc(m.rows, sizeof(double *));
	int i;
	for (i = 0; i < m.rows; ++i) m.vals[i] = (double *)calloc(m.cols, sizeof(double));
	return m;
}
...
matrix csv_to_matrix(char *filename, int header)
{
	...
	int n = 0;
	int size = 1024;
	m.vals = (double **)calloc(size, sizeof(double*));
	if (header) fgetl(fp);
	while ((line = fgetl(fp))) {
		if (m.cols == -1) m.cols = count_fields(line);
		if (n == size) {
			size *= 2;
			m.vals = (double **)realloc(m.vals, size * sizeof(double*));
		}
		m.vals[n] = parse_fields(line, m.cols);
		free(line);
		++n;
	}
	m.vals = (double **)realloc(m.vals, n * sizeof(double*));
	m.rows = n;
	return m;
}
